...
import {
  ...
  getSpotifyTrackId,
  LOGIN_FAIL_ERROR,
  SpotifySingleTrackSearchResult,
} from './utils/track';
import SpotifyWebApi from 'spotify-web-api-js';
import ConversionResult, {
  ConversionOutcome,
} from '../ConversionResult/ConversionResult';
...
const Converter = () => {
  ...
  const [conversionOutcome, setConversionOutcome] =
    useState<ConversionOutcome>('pending');
  ...
  const convertToSpotify = (token?: string) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.tabs.sendMessage(currentTabId, getTrackMessage, (response) =>
        onResponseFromContentScript(response, authToken)
      );
    });
  };
  ...
  const onResponseFromContentScript = async (
    ...
    const { spotifyTrackId, hasError: hasSearchError } =
      await fetchSpotifyTrackId(response.track, spotifyApi);
    ...
    const { hasError: hasAddToLikedSongsError } = await addTracksToLikedSongs(
      [spotifyTrackId],
      spotifyApi
    );
